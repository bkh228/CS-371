 ...
 * It sends a window of UDP packets, then waits for replies.
 * Packets not received before timeout are counted as lost.
 */
        ...
        while (received_in_batch < recv_target) {
            ...
            for (int j = 0; j < nfds; j++) {
                if (events[j].data.fd == data->socket_fd) {
                    ...
                    ssize_t bytes = recvfrom(data->socket_fd, recv_buf,
                        MESSAGE_SIZE, 0, NULL, NULL);
...
/*
 * Simple single-threaded UDP echo server using epoll.
 */
void run_server() {
    ...
    while (1) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (fd == listen_fd) {
                ...
                int bytes = recvfrom(listen_fd, buffer, MESSAGE_SIZE, 0,
                ...
                if (bytes < 0) {
                    ...
                    if (sendto(listen_fd, buffer, bytes, 0,
